...
instance ToRow CharPair where
    toRow cp = let cn = fst . unwrapCharPair $ cp
               ...
               in [toField cn, ci ^. hidden . to toField, ci ^. display . uni . to toField, ci ^. display . ids . to toField, ci ^. display . pua . to toField, ci ^. exact . cns . to toField, ci ^. exact . manualUni . to toField, ci ^. comments . to toField, ci ^. checked . to toField, ci ^. timestamp . to toField ]
